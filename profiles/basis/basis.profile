...
function basis_form_install_configure_form_alter(&$form, $form_state) {
  ...
  $form['site_information']['site_name']['#default_value'] = 'Basis';
	//On submit, execute basis_profile_install function
	$form['#submit'][] = '_basis_profile_install_configure_form_submit';
}

function _basis_profile_install_configure_form_submit($form, &$form_state) {
...
function basis_update_db($sql_file){
  global $databases;
...
function basis_install_tasks_alter(&$tasks, $install_state) {
  // Replace the install_finished installation task 
  $tasks['install_finished']['function'] = 'basis_install_finished';
}

function basis_install_finished(&$install_state) {
  drupal_set_title(st('@drupal installation complete', array('@drupal' => drupal_install_profile_distribution_name())), PASS_THROUGH);
  $messages = drupal_set_message();
  $output = '<p>' . st('Congratulations, you installed Basis!', array('@drupal' => drupal_install_profile_distribution_name())) . '</p>';
  ...
  // registered by the install profile are registered correctly.
	...
	// basis profile breaks if this is run
